...
Nenhuma classe aqui chama `audio` diretamente. Um minigame concreto pede
efeitos enfileirando o nome em `self.sfx_queue` (ver
`_BaseMinigame.queue_sfx`); `MinigameManager.drain_sfx()` esvazia essa
fila a cada quadro para `PuzzleSystem.update_minigame` tocar de verdade.
...
class _BaseMinigame:
    ...
    #: Chave curta que identifica o tipo de minigame (ver PuzzleSystem.update_minigame,
...
class MicroscopeMinigame(_BaseMinigame):
    ...
    `slots_state` é um dict (`PuzzleSystem.microscope_slots`, ver puzzles.py) que
    este minigame lê/escreve DIRETO — é por isso que fechar no meio com
    ESC não perde progresso: o dict já mora no sistema, só é passado por
    referência aqui."""
    ...
    def __init__(self, width, height, sprites_by_identity, complete_sprite, slots_state, key=None, body_sprite=None):
        ...
        # separada da Fase 1 — sem isso, PuzzleSystem.finish_minigame não teria
...
class EnergyBoxMinigame(_BaseMinigame):
    ...
    `state` (`PuzzleSystem.energy_box_state`) e `wires_state`
    (`PuzzleSystem.energy_box_wires`) são dicts passados por referência — mutados
