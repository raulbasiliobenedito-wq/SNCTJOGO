...
class _ChunkIndex(dict):
    """Dicionário de chunks com metadados das repetições removíveis."""

    def __init__(self):
        super().__init__()
        self.deduplicable_entry_ids = set()
...
class TiledMap:
    ...
    def __init__(self, path):
        ...
        # Cada item leva posição, direção e a máscara alpha do próprio tile.
        # Level._make_ramps combina as máscaras de uma mesma subida.
        self.tile_ramps = []
        self.object_groups = {}
        self.elapsed_ms = 0.0
        # Cache de tiles espelhados/girados (ver _oriented_image).
        self._oriented_cache = {}
        # Máscaras alpha dos tiles-rampa. São calculadas uma vez por imagem
        # e orientação e reaproveitadas por todas as instâncias do mapa.
        self._ramp_mask_cache = {}
        # Só imagens de tiles que atravessam chunks passam por esta análise.
        # O resultado permite remover blits repetidos sem mudar a composição.
        self._binary_alpha_cache = {}
    ...
    def _build_chunks(self, entries):
        ...
        for entry in entries:
            ...
            crosses_chunk = first_col != last_col or first_row != last_row
            if crosses_chunk and self._entry_has_binary_alpha(entry):
                chunks.deduplicable_entry_ids.add(id(entry))
        return chunks

    def _entry_has_binary_alpha(self, entry):
        """Confirma que repetir o blit é visualmente idempotente.

        Um tile com apenas pixels opacos ou transparentes pode ser mantido
        somente em sua última ocorrência na travessia dos chunks: o pixel
        opaco final substitui qualquer desenho anterior e o transparente não
        altera o destino. Com transparência parcial, os blits acumulam alpha;
        por isso essas entradas continuam repetidas como antes.
        """
        if len(entry) == 5:
            images = (entry[2],)
        else:
            tileset, local_id = entry[2], entry[3]
            images = (
                image
                for image, _duration in tileset["animation_frames"][local_id]
            )
        return all(self._image_has_binary_alpha(image) for image in images)

    def _image_has_binary_alpha(self, image):
        cache_key = id(image)
        cached = self._binary_alpha_cache.get(cache_key)
        if cached is not None:
            return cached

        surface_alpha = image.get_alpha()
        if surface_alpha not in (None, 255):
            result = False
        else:
            rgba = pygame.image.tostring(image, "RGBA")
            result = all(alpha in (0, 255) for alpha in rgba[3::4])
        self._binary_alpha_cache[cache_key] = result
        return result
    ...
    def _load_tile_ramps(tileset_root):
        ...
        "direita" ou "esquerda"). Um tile marcado assim usa seus próprios
        pixels alpha como forma sólida (ver ramp.Ramp) sempre que for pintado
        na camada de Colisão, em vez do quadrado cheio de sempre — sem
    ...
    def _read_tile_layer(self, layer):
        ...
        for index, raw_gid in enumerate(values):
            ...
            if creates_collision:
                ...
                if ramp_direction:
                    # A máscara vem do alpha do próprio tile, inclusive os
                    # degraus de pixel-art e pontas transparentes. Level
                    # combina máscaras vizinhas antes de criar a Ramp.
                    self.tile_ramps.append(
                        (
                            ...
                            self.tile_height,
                            ramp_direction,
                            self._ramp_collision_mask(tileset, local_id, flags),
                        )
                    )
                else:
    ...
    def _ramp_collision_mask(self, tileset, local_id, flags):
        """Converte os pixels visíveis do tile em sua forma de colisão.

        O limiar zero inclui qualquer pixel com alpha, em vez de aproximar
        a arte por um triângulo ideal. Se um tileset futuro usar escala, a
        máscara acompanha o tamanho da célula de colisão do mapa.
        """
        key = (id(tileset), local_id, flags, self.tile_width, self.tile_height)
        cached = self._ramp_mask_cache.get(key)
        if cached is not None:
            return cached
        image = (
            self._oriented_image(tileset, local_id, flags)
            if flags
            else self._tile_image(tileset, local_id)
        )
        cell_size = (self.tile_width, self.tile_height)
        if image.get_size() != cell_size:
            image = pygame.transform.scale(image, cell_size)
        collision_mask = pygame.mask.from_surface(image, 0)
        self._ramp_mask_cache[key] = collision_mask
    ...
    def _draw_chunks(self, surface, camera_x, camera_y, tile_chunks, animated_chunks):
        ...
        tile_ids = getattr(tile_chunks, "deduplicable_entry_ids", ())
        animated_ids = getattr(animated_chunks, "deduplicable_entry_ids", ())
        last_tile_chunk = self._last_visible_chunk(tile_chunks, visible_keys, tile_ids)
        last_animated_chunk = self._last_visible_chunk(
            animated_chunks, visible_keys, animated_ids
        )

        blit = surface.blit
        current = self._current_frames
        for key in visible_keys:
            for entry in tile_chunks.get(key, ()):
                x, y, image, width, height = entry
                entry_id = id(entry)
                if entry_id in tile_ids and last_tile_chunk[entry_id] != key:
                    continue
            ...
            for entry in animated_chunks.get(key, ()):
                x, y, tileset, local_id, width, height = entry
                entry_id = id(entry)
                if entry_id in animated_ids and last_animated_chunk[entry_id] != key:
                    continue
                if x + width < camera_x or x > right:
                    continue
                if y + height < camera_y or y > bottom:
    ...
    @staticmethod
    def _last_visible_chunk(chunks, visible_keys, deduplicable_ids):
        if not deduplicable_ids:
            return {}
        result = {}
        for key in visible_keys:
            for entry in chunks.get(key, ()):
                entry_id = id(entry)
                if entry_id in deduplicable_ids:
                    result[entry_id] = key
        return result
