...
class Level:
    ...
    # principal (ver Game.enter_room/InteractionSystem.use_doors) — não fazem parte da
    ...
    def __init__(self, index, room=None):
        ...
        # propriedade "rampa" no tileset vira uma máscara (ramp.Ramp) do
        # tamanho de uma célula sempre que for pintado na camada de
        # Colisão, sem precisar posicionar nada na camada Entidades. Ficam
        # de fora de self.grounds de propósito — colisão contra o alpha da
        # arte, não um AABB genérico (ver
        ...
        # InteractionSystem.use_secret_elevator/ElevatorCutscene) e
    ...
    def _make_lab_bench(self):
        ...
        PuzzleSystem.use_lab_microscope_bench/Game._release_fog_barrier). None até
    ...
    def _make_energy_box(self):
        ...
        PuzzleSystem.use_energy_box, que já lida com level.energy_box is
        None)."""
        if not self.tiled_map:
            return None
    ...
    def _make_ramps(self):
        ...
        "/"; "esquerda" — sobe da direita pra esquerda, tipo "\\") usa a
        máscara alpha do próprio tile, sem precisar posicionar nada à parte.
        ...
        bem na costura. Agora o merge também combina as máscaras alpha numa
        única forma, mantendo o contorno real sem reintroduzir as emendas."""
        if not self.tiled_map:
            return []
        return [
            Ramp(
                ...
                width,
                height,
                str(direction).strip().casefold() not in ("esquerda", "left"),
                collision_mask,
            )
            for x, y, width, height, direction, collision_mask
            in self._merge_ramp_tiles(self.tiled_map.tile_ramps)
        ]
    ...
    def _merge_ramp_tiles(tile_ramps):
        ...
        (x, y, width, height, direção, máscara), do início ao fim da corrente,
        sem nenhuma emenda no meio."""
        entries = []
        for item in tile_ramps:
            x, y, width, height, direction, *extra = item
            collision_mask = extra[0] if extra else None
            entries.append((x, y, width, height, direction, collision_mask))
        by_position = {(item[0], item[1]): item for item in entries}
        used = set()
        merged = []
        for key in by_position:
            if key in used:
                continue
            x, y, w, h, direction, _collision_mask = by_position[key]
            ...
            masks = [by_position[position][5] for position in chain]
            collision_mask = None
            if all(mask is not None for mask in masks):
                collision_mask = pygame.Mask((right - left, bottom - top))
                for position, mask in zip(chain, masks):
                    px, py = position
                    collision_mask.draw(mask, (px - left, py - top))
            merged.append(
                (left, top, right - left, bottom - top, direction, collision_mask)
            )
        return merged
    ...
    def _draw_energy_box(self, surface, camera_x, camera_y, energy_box_sprites, energy_box_state):
        ...
        (fechada/aberta/ligada), mesmos 3 estados que PuzzleSystem.energy_box_state
        guarda para o minigame. self.energy_box é None em qualquer mapa
